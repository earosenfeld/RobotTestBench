...
from robot_testbench.control import PIDController
from robot_testbench.daq import DataLogger, MotorTestMetadata
from datetime import datetime
from robot_testbench.control import PIDConfig
from robot_testbench.motor.motor_simulator import MotorController
...
def test_pid_control(motor_params, pid_gains):
    ...
    assert abs(final_pos - setpoint) < 10.0
...
def test_velocity_pid_control(motor_params, pid_gains):
    ...
    print(f"Max velocity: {max(velocity_history):.3f}, Min velocity: {min(velocity_history):.3f}")
    print(f"Max control: {max(control_history):.3f}, Min control: {min(control_history):.3f}")
    assert abs(final_vel - setpoint) < 10.0
