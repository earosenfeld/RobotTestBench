...
def test_force_torque_sensor():
    ...
    assert np.std(readings) < config.noise_std

def test_quadrature_encoder():
    """Test quadrature encoder features."""
...
def test_joint_angle_sensor():
    ...
    for _ in range(100):
        ...
        # Check quantization
        assert abs(reading % config.resolution) < 1e-10
    ...
    assert abs(reading1 - reading2) >= config.backlash
