...
def test_dyno_simulation(dyno_params):
    ...
    drive_voltage = 1.0  # 1V input to drive motor
    load_voltage = 0.0   # No voltage to load motor
    
    # Simulate for 1 second
    for _ in range(1000):
        drive_state, load_state = dyno.step(drive_voltage, load_voltage, dt)
...
def test_regenerative_braking(dyno_params):
    ...
    for _ in range(1000):
        dyno.step(1.0, 0.0, dt)  # Drive motor only
    ...
    for _ in range(1000):
        dyno.step(0.0, 0.5, dt)  # Load motor only
...
def test_coupling_behavior(dyno_params):
    ...
    # Apply different voltages to each motor
    for _ in range(1000):
        dyno.step(1.0, -0.5, dt)  # Drive forward, load backward
